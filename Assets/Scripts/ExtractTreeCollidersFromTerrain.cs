...
public class ExtractTreeCollidersFromTerrain : MonoBehaviour
{
    ...
    private void CreateNavMeshObstacle(TreePrototype tree, int prototypeIndex, TreeInstance[] instances, int instanceIndex)
    {
        ...
        var primitiveShape = CalculatePrimitiveShape(navMeshObstacle);
        var primitiveScale = CalculatePrimitiveScale(navMeshObstacle, instances[instanceIndex]);

        var obj = CreateGameObjectForNavMeshObstacle(tree, instances, instanceIndex, primitiveShape, primitiveScale);
        _createdObjects.Add(obj);
    }

    private GameObject CreateGameObjectForNavMeshObstacle(TreePrototype tree, TreeInstance[] instances, int instanceIndex, PrimitiveType primitiveShape, Vector3 primitiveScale)
    {
        var obj = GameObject.CreatePrimitive(primitiveShape);
    ...
    private PrimitiveType CalculatePrimitiveShape(NavMeshObstacle navMeshObstacle)
    {
    ...
    private Vector3 CalculatePrimitiveScale(NavMeshObstacle navMeshObstacle, TreeInstance instance)
    {
        ...
        if (navMeshObstacle.shape == NavMeshObstacleShape.Capsule)
            return navMeshObstacle.radius * scale;

        return new Vector3(
            navMeshObstacle.size.x * scale.x,
            navMeshObstacle.size.y * scale.y,
            navMeshObstacle.size.z * scale.z
        );
    }
